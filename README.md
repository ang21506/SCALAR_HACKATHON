...
```bash
# Required by hackathon evaluator
export API_BASE_URL="https://your-openai-compatible-endpoint/v1"
export MODEL_NAME="your-model-name"

# Required for LLM baseline, optional for rule-based only runs
...
To deploy this environment to Hugging Face Spaces, push this repository to your Space. The container runs `inference.py` directly (no web UI), which is aligned with Round 1 baseline validation.
...
## Round 1 Compliance Notes
- Inference entrypoint is `inference.py` at repository root.
- All LLM calls use `from openai import OpenAI` with `API_BASE_URL`, `MODEL_NAME`, and `HF_TOKEN`.
- Inference emits structured `[START]`, `[STEP]`, and `[END]` logs for each episode.
- The environment provides typed `observation`, `action`, `reward`, and `state` schemas via `openenv.yaml`.
- Tasks include `task1_easy`, `task2_medium`, and `task3_hard`, each graded on a `0.0-1.0` score via `env.grade()`.
- UI has been removed from this repository to keep the submission focused on OpenEnv API + inference validation.
